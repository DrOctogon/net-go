...
func (c *Controller) HandleSearch(ctx echo.Context) error {
	...
	c.logInfoIfEnabled("Handling search request", logger.String("path", path), logger.IP("ip", ip))

	// Parse the request
	var req SearchRequest
	if err := ctx.Bind(&req); err != nil {
		c.logErrorIfEnabled("Failed to bind search request", logger.Error(err), logger.String("path", path), logger.IP("ip", ip))
	...
	c.logDebugIfEnabled("Executing search with filters", logger.Any("filters", filters), logger.String("path", path), logger.IP("ip", ip))

	// Execute the search
	results, total, err := c.DS.SearchDetections(&filters)
	if err != nil {
		c.logErrorIfEnabled("Search query failed", logger.Error(err), logger.String("filters", fmt.Sprintf("%+v", filters)), logger.String("path", path), logger.IP("ip", ip))
...
func (c *Controller) validateAndNormalizeSearchRequest(ctx echo.Context, req *SearchRequest) error {
	...
	// Validate Page
	if req.Page < 1 {
		c.logWarnIfEnabled("Invalid page number requested, defaulting to 1", logger.Int("requested_page", req.Page), logger.String("path", path), logger.IP("ip", ip))
		req.Page = 1
	}
...
func (c *Controller) validateSearchDates(path, ip string, req *SearchRequest) error {
	if err := validateDateFormat(req.DateStart, "start date"); err != nil {
		c.logErrorIfEnabled("Invalid start date format", logger.String("dateStart", req.DateStart), logger.String("path", path), logger.IP("ip", ip))
		return err
	}
	if err := validateDateFormat(req.DateEnd, "end date"); err != nil {
		c.logErrorIfEnabled("Invalid end date format", logger.String("dateEnd", req.DateEnd), logger.String("path", path), logger.IP("ip", ip))
		return err
	}
	if err := validateDateOrder(req.DateStart, req.DateEnd); err != nil {
		c.logErrorIfEnabled("Invalid date range", logger.String("dateStart", req.DateStart), logger.String("dateEnd", req.DateEnd), logger.String("path", path), logger.IP("ip", ip))
...
func (c *Controller) validateSearchStatusEnums(path, ip string, req *SearchRequest) error {
	...
	if req.VerifiedStatus == "" {
		...
		c.logErrorIfEnabled("Invalid verified status parameter", logger.String("verifiedStatus", req.VerifiedStatus), logger.String("path", path), logger.IP("ip", ip))
	...
	if req.LockedStatus == "" {
		...
		c.logErrorIfEnabled("Invalid locked status parameter", logger.String("lockedStatus", req.LockedStatus), logger.String("path", path), logger.IP("ip", ip))
...
func (c *Controller) validateSearchTimeOfDay(path, ip string, req *SearchRequest) error {
	...
	if req.TimeOfDay == "" {
		...
		c.logErrorIfEnabled("Invalid time of day parameter", logger.String("timeOfDay", req.TimeOfDay), logger.String("path", path), logger.IP("ip", ip))
...
func (c *Controller) normalizeConfidenceMax(minConf, maxConf float64, path, ip string) float64 {
	...
	case maxConf == 0 && minConf == 0:
		c.logDebugIfEnabled("Confidence range is [0, 0], defaulting ConfidenceMax to 1", logger.String("path", path), logger.IP("ip", ip))
		return 1
	default:
		return maxConf
	}
...
func (c *Controller) logConfidenceAdjustment(field string, original, adjusted float64, path, ip string) {
	c.logWarnIfEnabled("Invalid "+field+", adjusted", logger.Float64("original", original), logger.Float64("adjusted", adjusted), logger.String("path", path), logger.IP("ip", ip))
}
...
func (c *Controller) logConfidenceSwap(minConf, maxConf float64, path, ip string) {
	c.logWarnIfEnabled("ConfidenceMin > ConfidenceMax after normalization, swapping values",
		logger.Float64("normalizedConfidenceMin", minConf), logger.Float64("normalizedConfidenceMax", maxConf), logger.String("path", path), logger.IP("ip", ip))
}
...
func (c *Controller) validateSearchSortBy(path, ip string, req *SearchRequest) error {
	...
	if req.SortBy != "" { // Allow empty string for default sorting (handled by datastore)
		if _, ok := allowedSortBy[req.SortBy]; !ok {
			c.logErrorIfEnabled("Invalid sortBy parameter", logger.String("sortBy", req.SortBy), logger.String("path", path), logger.IP("ip", ip))
