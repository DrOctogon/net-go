# Exposing VoiceWatch to the Internet with Cloudflare Tunnel

This guide explains how to securely expose your VoiceWatch installation to the internet using Cloudflare Tunnel (cloudflared), which is the recommended method for remote access.
...
- VoiceWatch running on your system (any installation method is compatible)
...
If you're using Docker Compose with VoiceWatch, setting up Cloudflare Tunnel is straightforward:

1. **Create a Cloudflare Tunnel**:
   - Log in to the [Cloudflare Zero Trust dashboard](https://dash.teams.cloudflare.com/)
   - Navigate to Access > Tunnels
   - Click "Create a tunnel"
   - Give your tunnel a name (e.g., "VoiceWatch")
...
Your VoiceWatch instance will now be accessible at `https://birdnet.yourdomain.com` from anywhere.
...
If you're running VoiceWatch directly on your system (not using Docker):
...
If you want to quickly test exposing your VoiceWatch instance without configuring a domain or creating a permanent tunnel, you can use Cloudflare's "quick tunnels" feature:
...
This will create a temporary public URL that you can use to access your VoiceWatch instance. The output will show the random hostname assigned to your tunnel, such as `https://randomly-generated-hostname.trycloudflare.com`.
...
When exposing VoiceWatch to the internet, it's **strongly recommended** to enable authentication.

### Security Implications of Not Enabling Authentication

Without authentication, your VoiceWatch instance is completely open to anyone who has the URL. This creates significant security and privacy risks:
...
Even with Cloudflare Tunnel's security benefits, exposing an unauthenticated VoiceWatch instance to the internet is equivalent to leaving your front door wide open. The tunnel provides secure access, but without authentication, anyone who finds your URL has complete control over your VoiceWatch installation.

### Setting Up Authentication

There are several authentication methods available in VoiceWatch:
...
2. **Restart VoiceWatch** for the changes to take effect
...
If you're sharing your VoiceWatch instance with family or friends, consider using OAuth which makes it easier to control who has access without sharing passwords.
...
- **Connection refused errors**: Verify that the VoiceWatch container is accessible from the cloudflared container. They should be on the same Docker network.
